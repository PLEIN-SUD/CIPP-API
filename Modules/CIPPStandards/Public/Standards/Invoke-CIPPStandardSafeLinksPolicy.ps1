function Invoke-CIPPStandardSafeLinksPolicy {
    ...
    $PolicyName = if ($Settings.name) { $Settings.name } else { 'Plein Sud IT - Default SafeLinks Policy' }
    $PolicyList = @($PolicyName, 'Plein Sud IT - efault SafeLinks Policy', 'Default SafeLinks Policy')
    ...
    $RuleList = @($DesiredRuleName, "$PolicyName Rule", 'Plein Sud IT - Default SafeLinks Rule', 'Plein Sud IT - Default SafeLinks Policy')
