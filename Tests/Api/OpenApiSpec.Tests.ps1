...
Describe 'coverage against the entrypoint sources' {
    ...
    It 'documents every function marked Entrypoint' {
        ...
        # being projected into the MCP tool list is the intended policy for them anyway. The match
        # covers the Public prefix too: PublicPSITSocWebhook is the same fork endpoint, renamed to
        # cross the static web app's anonymous route.
        $Missing = @($script:Sources | Where-Object { $_.Endpoint -notmatch '^(Public)?PSIT' -and -not $script:Spec.paths.ContainsKey("/api/$($_.Endpoint)") })
        # PSIT-CUSTOM-END
