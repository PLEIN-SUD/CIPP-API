...
Describe 'Test-PSITBecServiceIp' {
    ...
    It 'leaves a real user address alone' {
        Test-PSITBecServiceIp -Ip '92.92.126.129' | Should -BeFalse
        Test-PSITBecServiceIp -Ip '146.241.181.10' | Should -BeFalse
    }
...
Describe 'Get-PSITBecOutboundClassification' {
    BeforeEach {
        $script:Rows = @(
            ...
            New-TraceRow 'Réponse automatique : PMO REGION TOULOUSAINE' 'agnes@cimpa.test' '2603:10a6:803:81::32' '2026-08-20T10:13:07Z'
            ...
            New-TraceRow 'Undeliverable: EVP Aix' 'nobody@client.test' '2603:10a6:803:81::32' '2026-08-20T10:13:10Z'
            # Internal mail from the user's own workstation.
            New-TraceRow 'Re: EVP Aix aout 2026' 'a.servanty@geser.test' '92.92.126.129' '2026-08-20T10:07:48Z'
            New-TraceRow 'Re: EVP Aix aout 2026' 's.baji@geser.test' '92.92.126.129' '2026-08-20T10:07:48Z'
            # Genuine external mail.
            New-TraceRow 'Fwd: consultation AO2606960' 'contact@sopra.test' '92.92.126.129' '2026-08-20T10:03:08Z'
        )
    }

    It 'separates service, internal and analysable traffic' {
        $Result = Get-PSITBecOutboundClassification -TraceRows $script:Rows -SenderAddress 'p.taieb@geser.test'
    ...
    It 'still flags a real burst of external mail' {
        ...
        $Result = Get-PSITBecOutboundClassification -TraceRows $Burst -SenderAddress 'p.taieb@geser.test'
    ...
    It 'flags a repeated subject sent to many external recipients' {
        ...
        $Result = Get-PSITBecOutboundClassification -TraceRows $Campaign -SenderAddress 'p.taieb@geser.test'
    ...
    It 'never flags a campaign built only out of automatic replies' {
        ...
        $Result = Get-PSITBecOutboundClassification -TraceRows $AutoOnly -SenderAddress 'p.taieb@geser.test'
    ...
    It 'returns empty counters rather than throwing on no trace at all' {
        $Result = Get-PSITBecOutboundClassification -TraceRows @() -SenderAddress 'p.taieb@geser.test'
